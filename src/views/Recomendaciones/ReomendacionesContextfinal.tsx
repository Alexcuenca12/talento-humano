...
function Recomendaciones() {
  ...
  const handleGuardarClick = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/api/recomendaciones/create",
        {
          method: "POST",
          headers: {
  ...
  const obtenerRecomendaciones = async () => {
    try {
      const response = await fetch(
        "http://localhost:8080/api/recomendaciones/read"
      );
      if (response.ok) {
          ...
          <form className="formulario">
            <div className="form-row">
              <div className="input-container">
                <label className="etiqueta" htmlFor="level">
                  Nombres:
                </label>
                <InputText
                ...
                <InputText
                  ...
                  onChange={
                    editMode
                      ...
                      : handlepapellidoChange
                  }
                />
              </div>
              <div className="input-container">
                <label className="etiqueta" htmlFor="institution">
                  Email:
                </label>
                <InputText
                  id="email"
            ...
            <div className="form-row">
              <div className="input-container">
                <label className="etiqueta" htmlFor="duration">
                  Telefono:
                </label>
                <InputText
                  className="small-input"
  ...
  />
              ...
              <tbody>
                {reco1.map((reco) => (
                  <tr key={reco.id_recomendaciones?.toString()}>
                    ...
                    <button
                      onClick={() =>
                        handleDelete(reco.id_recomendaciones?.valueOf())
                      }
                    >
                      Eliminar
                    </button>
                    <button
                      onClick={() =>
                        handleEdit(reco.id_recomendaciones?.valueOf())
                      }
                    >
                      Editar
                    </button>
          ...
          <div className="">
            <Button
              type="button"
              label="Continuar"
              className="small-button p-button-secondary additional-button"
              style={{ background: "#0C3255" }}
            />
          </div>
        </div>
