...
export default function CapacitacionesContext() {
    ...
    const [fechaInicio, setFechaInicio] = useState<string | Date | Date[] | null>(null);
    ...
    const [editingCapacitaciones, setEditingCapacitaciones] = useState<ICapacitaciones | null>(null);
    ...
    const fecthCapacitaciones = () => {
        new CapacitacionesService().getAllCap()
            .then((data) => {
    ...
    const [selectCapacitacionesI, setSelectCapacitacionesI] = useState<ICapacitaciones | null>(null);
    ...
    const handleSubmit = (e: React.SyntheticEvent) => {
        ...
        const fechaInicioString: string = "";
        const fechaFinString: string = "";
        ...
        if (selectCapacitacionesI) {
            const updatedCapacitaciones: ICapacitaciones = {
                ...
                fecha_inicio:  Array.isArray(fechaInicio) && fechaInicio.length > 0 ? new Date(fechaInicio[0]) : null,
                fecha_fin: Array.isArray(fechaFin) && fechaFin.length > 0 ? new Date(fechaFin[0]) : null ,
            ...
            const newCapacitaciones: ICapacitaciones = {
                ...
                area_estudios: selectArea ?? '',
                tipo_certificado: selectCertificado ?? '',
                fecha_inicio: new Date(fechaInicioString),
                fecha_fin: new Date(fechaFinString) ,
                numero_dias: numeroDias ?? 0 ,
                cantidad_horas: cantidadHoras ?? 0,
                persona: persona
            };
    ...
    const editCapacitacion = (rowData: ICapacitaciones) => {
        ...
        setFechaInicio(rowData.fecha_inicio ? new Date(rowData.fecha_inicio.toString()) : null);
        setFechaFin(rowData.fecha_fin ? new Date(rowData.fecha_fin.toString()) : null);
                                    ...
                                    <div className="flex flex-column">
                                        <div
                                            ...
                                            <div
                                                ...
                                                <InputText type="text" placeholder="Ingrese la Institucion"
                                                           className="w-auto min-w-min text-2xl" value={institucion}
                                                           onChange={({target})=> setInstitucion(target.value)}/>
                                            </div>
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2">
                                                <label className="text-3xl font-medium w-auto min-w-min">Tipo de
                                                    evento:</label>
                                                <Dropdown options={comboEvento} value={selectEvento}
                                            ...
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2 ">
                                                <label className="text-3xl font-medium w-auto min-w-min">Area de
                                                    estudio:</label>
                                                <Dropdown options={comboArea} value={selectArea}
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <Calendar className=" w-full min-w-min "
                                                          ...
                                                          dateFormat="dd/mm/yy"
                                                          value={fechaInicio}
                                                          onChange={(e) => setFechaInicio(e.value as Date | null)}
                                                          showIcon/>
                                            </div>
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2">
                                            ...
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2 ">
                                                <label className="text-3xl font-medium w-full  min-w-min">Numero de
                                                    dias:</label>
                                                <InputNumber placeholder="Ingrese los dias"
                                                           className="w-auto min-w-min text-2xl"
                                                             value={numeroDias}
                                                             onValueChange={(e: InputNumberValueChangeEvent) => setNumeroDias(e.value as number)}/>
                                            </div>
                                        </div>
                                        ...
                                        <div
                                            ...
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2 h-7rem">
                                                <label className="text-3xl font-medium w-full  min-w-min">Nombre de
                                                    evento:</label>
                                                <InputText type="text" placeholder="Ingrese el nombre"
                                                           className="w-auto min-w-min text-2xl"
                                                           value={nombreEvento || undefined}
                                                           onChange={({target})=> setNombreEvento(target.value)}/>

                                            </div>
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2">
                                                <label className="text-3xl font-medium w-full  min-w-min">Tipo de
                                                    certificado:</label>
                                                <Dropdown options={comboCertificado} value={selectCertificado}
                                            ...
                                            <div
                                                className="flex align-items-center justify-content-center w-auto pr-2 ">
                                                <label className="text-3xl font-medium w-full  min-w-min">N° de horas
                                                    totales: </label>
                                                <InputNumber placeholder="Ingrese los dias"
                                                             className="w-auto min-w-min text-2xl"
                                                             value={cantidadHoras}
                                                             onValueChange={(e: InputNumberValueChangeEvent) => setCantidadHoras(e.value as number)}/>
                                ...
                                body={(rowData: ICapacitaciones) => (
                                    <>
                                        ...
                                        <Button icon="pi pi-trash" className="p-button-rounded p-button-danger"
                                                onClick={() => eliminarCapacitaciones(rowData.id_capacitaciones)}/>
                                    </>
                                )}
                        />
